...
const thoughtController = {
  async getThoughts(req, res) {
    try {
      const dbDataThought = await Thought.find().sort({ createdAt: -1 });
  ...
  async getSingleThought(req, res) {
    try {
      const dbDataThought = await Thought.findOne({
  ...
  async createThought(req, res) {
    try {
      ...
      if (!dbDataUser) {
        return res
          .status(404)
          .json({ message: "Thought created but no user associated!" });
      }
  ...
  async updateThought(req, res) {
    try {
      ...
      if (!dbDataThought) {
        return res
          .status(404)
          .json({ message: "No thought associated with the id!" });
      }
      res.json(dbDataThought);
    } catch (err) {
      console.log(err);
  ...
  async deleteThought(req, res) {
    try {
      const dbDataThought = await Thought.findOneAndRemove({
        _id: req.params.thoughtId,
      });
      if (!dbDataThought) {
